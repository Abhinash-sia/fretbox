...
import { useFacilityAssets, useDeleteAsset } from '@/features/facilities/hooks/use-facilities-queries';
import { CreateAssetModal } from '@/features/facilities/components/create-asset-modal';
import { FacilityAsset, AssetCategory, AssetStatus, AssetCondition } from '@/features/facilities/types/facilities';

export default function WardenFacilitiesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Are you sure you want to delete this facility asset?')) {
      try {
  ...
  return (
    <ProtectedRoute allowedRoles={['warden', 'administrator', 'staff']}>
      <AppShell>
        <PageHeader
          heading="Facility Assets & Inventory Management"
          subheading="Register campus assets, track operational condition, and manage maintenance logs."
        >
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  ...
                  <span>Campus Facility Asset Records ({assets.length})</span>
                </CardTitle>
                <CardDescription className="text-xs">
                  Operational condition, asset tag numbers, and location mapping.
                </CardDescription>
              </CardHeader>
                ...
                {assets.length > 0 ? (
                  ...
                  <div className="p-6">
                    <EmptyState
                      ...
                      description="No assets match the selected category or search filter."
                      actionLabel="Register Asset"
                      onAction={() => { setAssetToEdit(null); setCreateModalOpen(true); }}
                    />
                  </div>
                )}
              </CardContent>
